"use strict";
/* ============================================================
   student-courses.js — Student course list + key enrollment
   FIXED: element IDs now match student-courses.html
============================================================ */
...
function loadNotifBadge() {
  fetch("/notifications/count", { credentials: "include" })
    ...
    .then(d => {
      ...
      if (el && d.count > 0) { el.innerText = d.count; el.style.display = "inline"; }
    }).catch(() => {});
}

/* ── TABS ── */
function showTab(tab, btn) {
  ["enrolled", "browse"].forEach(t => {
...
function loadMyEnrollments() {
  fetch("/courses/my", { credentials: "include" })
    ...
    .then(courses => {
      ...
      renderEnrolledGrid(courses);
    })
    .catch(err => {
      console.error("loadMyEnrollments error:", err);
      const el = document.getElementById("enrolledGrid");
      if (el) el.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><span class="empty-icon">⚠️</span><p>Failed to load courses. Make sure you're logged in.</p></div>`;
    });
}
...
function renderAlerts(courses) {
  ...
  if (!sec) return;
  const low = courses.filter(c =>
    (c.lec_pct !== null && c.lec_pct < 75) ||
    (c.lab_pct !== null && c.lab_pct < 75)
  );
  ...
  if (!courses.length) {
    el.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;">
        ...
        <button class="btn btn-primary" onclick="openEnrollModal()">🔑 Enroll with Key</button>
      </div>`;
    return;
  }
    ...
    return `
      <div class="my-course-card">
        ...
        <div class="my-course-body">
          ...
          ${(!c.lec_pct && !c.lab_pct) ? `
            <p style="color:var(--muted);font-size:13px;margin-bottom:12px;">No attendance recorded yet</p>` : ""}

          ${c.pending_assignments > 0 ? `
            <div style="margin-bottom:12px;">
              <span class="assign-chip">📝 ${c.pending_assignments} pending assignment${c.pending_assignments !== 1 ? "s" : ""}</span>
            </div>` : ""}

          <div class="course-action-row">
            <button class="btn btn-outline btn-sm" onclick="viewAttendance(${c.id}, '${esc(c.name)}')">📊 Attendance</button>
            <button class="btn btn-outline btn-sm" onclick="viewAssignments(${c.id}, '${esc(c.name)}')">📝 Assignments</button>
            <button class="btn btn-danger btn-sm" onclick="unenroll(${c.id}, '${esc(c.name)}')">Leave</button>
          </div>
        </div>
      </div>`;
  }).join("");
...
function attClass(pct) {
  if (pct === null || pct === undefined || pct >= 75) return "good";
...
function loadBrowseCourses() {
  ...
  if (el) el.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><span class="empty-icon">🔍</span><p>Loading courses...</p></div>`;
  ...
  fetch("/courses/", { credentials: "include" })
    ...
    .catch(() => {
      const el = document.getElementById("browseGrid");
      if (el) el.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><p style="color:red;">Failed to load courses</p></div>`;
    });
}
...
function renderBrowseGrid(courses) {
  ...
  if (!courses.length) {
    el.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><span class="empty-icon">🔍</span><p>No courses found</p></div>`;
    return;
  }
  ...
  el.innerHTML = courses.map(c => {
    ...
    return `
      <div class="browse-card ${enrolled ? "enrolled" : ""}">
        ${c.code ? `<div class="course-code">${esc(c.code)}</div>` : ""}
        <div class="course-name">${esc(c.name)}</div>
        <div class="course-meta">
          👨‍🏫 ${esc(c.faculty_name || "—")}
          ${c.semester ? " · Sem " + c.semester : ""}
          ${c.description ? `<br><span style="font-size:12px;">${esc(c.description.slice(0, 80))}${c.description.length > 80 ? "…" : ""}</span>` : ""}
        </div>
        <div class="course-footer">
          <span style="font-size:12px;color:var(--muted);">👨‍🎓 ${c.enrolled_count || 0} enrolled</span>
          ${enrolled
            ? `<span class="enrolled-label">✅ Enrolled</span>`
            : `<button class="btn btn-primary btn-sm" onclick="openEnrollModal()">🔑 Enroll</button>`
          }
        </div>
      </div>`;
  }).join("");
...
function filterBrowse() {
  ...
  filteredAll = allCourses.filter(c => {
    ...
    const matchSem = !sem || String(c.semester) === sem;
...
function viewAttendance(courseId, courseName) {
  ...
  fetch(`/courses/${courseId}/my-attendance`, { credentials: "include" })
    ...
    .then(sessions => {
      if (!sessions.length) {
        body.innerHTML = `<p style="color:var(--muted);text-align:center;padding:20px;">No attendance sessions recorded yet.</p>`;
        return;
      }
      ...
      body.innerHTML = `
        <div class="session-summary">
          <div class="session-summary-item session-summary-present">
            <div class="summary-big-num">${pct}%</div>
            <div class="summary-item-label">Overall</div>
          </div>
          <div class="session-summary-item session-summary-present">
            <div class="summary-big-num">${present}</div>
            <div class="summary-item-label">Present</div>
          </div>
          <div class="session-summary-item session-summary-late">
            <div class="summary-big-num">${late}</div>
            <div class="summary-item-label">Late</div>
          </div>
          <div class="session-summary-item session-summary-absent">
            <div class="summary-big-num">${absent}</div>
        ...
        <div style="max-height:350px;overflow-y:auto;">
          <table class="session-table">
            <thead>
              <tr>
                <th>Date</th><th>Type</th><th>Topic</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              ${sessions.map(s => `
                <tr>
                  <td>${s.session_date ? String(s.session_date).slice(0,10) : "—"}</td>
    ...
    .catch(() => {
      body.innerHTML = `<p style="color:red;text-align:center;padding:20px;">Failed to load attendance data.</p>`;
    });
}
...
/* ── ASSIGNMENTS MODAL ── */
function viewAssignments(courseId, courseName) {
  ...
  fetch(`/courses/${courseId}/assignments`, { credentials: "include" })
    ...
    .then(assignments => {
      if (!assignments.length) {
        body.innerHTML = `<p style="color:var(--muted);text-align:center;padding:20px;">No assignments for this course yet.</p>`;
        return;
      }
          ...
          <div style="padding:16px;border-radius:12px;background:var(--bg);border:1px solid var(--border);margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;flex-wrap:wrap;">
              <div style="flex:1;min-width:0;">
                ...
                ${a.marks !== null && a.marks !== undefined ? `<div style="margin-top:6px;font-size:13px;color:var(--green);font-weight:700;">🎯 Marks: ${a.marks}${a.max_marks ? "/" + a.max_marks : ""}</div>` : ""}
              </div>
              <div>${statusLabel}</div>
            </div>
          </div>`;
    ...
    .catch(() => {
      body.innerHTML = `<p style="color:red;text-align:center;padding:20px;">Failed to load assignments.</p>`;
    });
}
...
function openEnrollModal() {
  const modal = document.getElementById("enrollKeyModal");
...
function enrollByKey() {
  // The HTML uses "enrollKeyInput" and "enrollKeyMsg"
  const input = document.getElementById("enrollKeyInput");
  const msg   = document.getElementById("enrollKeyMsg");
  const key   = (input?.value || "").trim().toUpperCase();

  if (!key) { showMsg(msg, "error", "⚠️ Please enter an enrollment key"); return; }
  showMsg(msg, "success", "⏳ Enrolling…");

  fetch("/courses/enroll-by-key", {
    method: "POST",
    credentials: "include",
  ...
  .then(d => {
    if (d.success) {
      showMsg(msg, "success", `✅ Enrolled in "${d.course_name}"!`);
      setTimeout(() => {
        closeEnrollModal();
        loadMyEnrollments();
        // Also refresh browse tab if it's loaded
        if (allCourses.length) loadBrowseCourses();
      }, 1200);
    } else {
      showMsg(msg, "error", "❌ " + (d.message || "Invalid key or already enrolled"));
    }
  })
...
/* ── UNENROLL ── */
function unenroll(courseId, courseName) {
  if (!confirm(`Leave course "${courseName}"?\n\nYour attendance history will be preserved.`)) return;

  fetch("/courses/unenroll", {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
  ...
  .then(d => {
    if (d.success) {
      ...
      alert("❌ " + (d.message || "Could not unenroll"));
    }
  })
